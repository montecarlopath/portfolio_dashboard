...
import { useHedgeDashboardBundle } from "../hooks/useHedgeDashboardData";
...
export default function EodReviewDashboard() {
    ...
    const { data: bundle, isLoading } = useHedgeDashboardBundle("all");

    const load = () => {
        qc.invalidateQueries({ queryKey: ["hedge-dashboard-bundle"] });
    };
    ...
    const tickets = bundle?.tickets_preview || {};
    const history = bundle?.history_30d || {};
    const crash = bundle?.crash_sim || {};
    const context = bundle?.context || {};

    // Bundle does not currently return order history or eod alerts.
    const orders = { orders: [], filled: 0, total_actual_debit_dollars: 0 };
    const eodAlerts = { alerts: [] };
        ...
        <div style={{ padding: "1rem 0", fontFamily: "var(--font-sans)" }}>
            ...
            >
                <div>
                    ...
                    >
                        {intel.as_of_date || new Date().toISOString().split("T")[0]} · refreshed{" "}
                        {refreshed}
                    </div>
                </div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <Badge
                        type={alerts.length > 0 ? "warn" : "ok"}
                        label={alerts.length > 0 ? `${alerts.length} alert` : "No alerts"}
                    />
                    <Badge
                        type={openOrders.length > 0 ? "warn" : gapMet ? "ok" : "warn"}
                        label={openOrders.length > 0 ? `${openOrders.length} open` : "No open orders"}
                    />
            ...
            <Card title="market_signal_read + market_regime_read" layer="L1" status={intel._error ? "err" : "ok"}>
                <Grid cols={4}>
                    <Metric label="QQQ spot" value={context.qqq_spot != null ? fmtN(context.qqq_spot, 2) : "—"} />
                    <Metric
                        label="Regime"
                        value={regime}
                        color={
                    ...
                    <Metric
                        label="Risk score"
                        value={fmtN(intel.market_risk_score, 2)}
                        color={
                            intel.market_risk_score > 0.6
                                ? "#D97706"
                                : "var(--color-text-primary)"
                        }
                    />
            ...
            <Card title="portfolio_risk_read + holdings" layer="L1" status={intel._error ? "err" : "ok"}>
                ...
                <Grid cols={2}>
                    <Metric
                        ...
                        value={fmt$(intel.structural_hedge_exposure_dollars)}
                        sub="PSQ + SQQQ + UVXY (already in beta)"
                    />
                    <Metric
                        label="Option hedge on"
                        value={fmt$(intel.option_hedge_exposure_dollars)}
                        sub={`${fmtPct(intel.option_hedge_exposure_pct)} of portfolio`}
                    />
                </Grid>
            </Card>
            ...
            <Card title="hedge_intelligence_read — gap calculation" layer="L2" status={gapMet ? "ok" : "warn"}>
                <Grid cols={4}>
                    <Metric
                        ...
                        sub={fmt$(intel.current_hedge_exposure_dollars)}
                        color={gapMet ? "var(--color-text-success)" : "#D97706"}
                    />
                    <Metric
                        label="Recommended"
                        value={fmtPct(intel.recommended_hedge_pct)}
                        sub={fmt$(intel.recommended_hedge_exposure_dollars)}
                    />
                    <Metric
                        label="Gap to fill"
                        value={fmtPct(intel.additional_hedge_pct)}
                        sub={fmt$(intel.additional_hedge_exposure_dollars)}
                        color={gapMet ? "var(--color-text-success)" : "#DC2626"}
                    />
                    <Metric
                        label="Budget left"
                        value={fmt$(intel.remaining_hedge_budget_dollars)}
                        sub={`${fmtPct(intel.remaining_hedge_budget_pct)} of portfolio`}
                    />
                </Grid>
                ...
                >
                    ...
                    >
                        theoretical target = {fmt$(intel.theoretical_recommended_hedge_exposure_dollars)} ·
                    ...
                    <Grid cols={2}>
                        <div>
                            ...
                            <Row
                                label="Long bid/ask"
                                value={
                                    sel.primary_spread.long_leg
                                        ? `${sel.primary_spread.long_leg.bid}/${sel.primary_spread.long_leg.ask}`
                                        : "—"
                                }
                            />
                            <Row
                                label="Long delta"
                                value={fmtN(sel.primary_spread.long_leg?.delta, 3)}
                            />
                            <Row
                                label="Short delta"
                                value={fmtN(sel.primary_spread.short_leg?.delta, 3)}
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <Row
                                label="Long bid/ask"
                                value={
                                    sel.tail_spread?.long_leg
                                        ? `${sel.tail_spread.long_leg.bid}/${sel.tail_spread.long_leg.ask}`
                                        : "—"
                                }
                            />
                            <Row
                                label="Long delta"
                                value={fmtN(sel.tail_spread?.long_leg?.delta, 3)}
                            />
                            <Row
                                label="Short delta"
                                value={fmtN(sel.tail_spread?.short_leg?.delta, 3)}
                            />
                        </div>
                    </Grid>
                )}
            ...
            <Card title="hedge_execution_planner — contract sizing" layer="L4" status={plan.primary_spread ? "ok" : "warn"}>
                <Grid cols={3}>
                    <div>
                        ...
                        <Row
                            label="Debit/contract"
                            value={
                                plan.primary_spread?.debit_per_contract != null
                                    ? `$${plan.primary_spread.debit_per_contract.toFixed(0)}`
                                    : "—"
                            }
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <Row
                            label="Debit/contract"
                            value={
                                plan.tail_spread?.debit_per_contract != null
                                    ? `$${plan.tail_spread.debit_per_contract.toFixed(0)}`
                                    : "—"
                            }
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <Row
                            label="Coverage/cost"
                            value={
                                plan.total_estimated_cost_dollars > 0
                                    ? `${(plan.total_estimated_hedge_dollars / plan.total_estimated_cost_dollars).toFixed(1)}×`
                                    : "—"
                            }
                        />
                    </div>
            ...
            <Card
                title="hedge_reconciliation_engine — current vs target"
                layer="L4"
                status={roll.summary_action === "hold" ? "ok" : "warn"}
            >
                <Grid cols={2}>
                    <div>
                        <div
                        ...
                        <Row
                            label="Positions"
                            value={recon.primary_action?.current_positions?.join(", ") || "none"}
                        />
                        <div
                            style={{
                                fontSize: 11,
                    ...
                    <div>
                        ...
                        <Row
                            label="Positions"
                            value={recon.tail_action?.current_positions?.join(", ") || "none"}
                        />
                        <div
                            style={{
                                fontSize: 11,
                        ...
                        >
                            {recon.tail_action?.reason || ""}
                        </div>
                    </div>
                </Grid>

                <div style={{ marginTop: 8, display: "flex", gap: 8 }}>
                    {(recon.immediate_actions || []).map((a, i) => (
                        <Badge key={i} type="warn" label={a} />
                    ))}
                    {(recon.immediate_actions || []).length === 0 && (
                        <Badge type="ok" label="No immediate actions" />
                    )}
                </div>
            </Card>
            ...
            <Card
                title="hedge_trade_ticket_engine — tickets generated"
                layer="L5"
                status={executableTickets.length > 0 ? "warn" : holdTickets.length > 0 ? "ok" : "idle"}
            >
                {ticketList.length === 0 ? (
                            ...
                            >
                                ...
                                <div style={{ textAlign: "right" }}>
                                    {t.estimated_debit_dollars > 0 && (
                                        ...
                                        >
                                            {fmt$(t.estimated_debit_dollars)}
                                        </span>
                                    )}
                                    {t.estimated_coverage_added_dollars > 0 && (
                                        <span
                                            style={{
                                                fontSize: 11,
                                                color: "var(--color-text-secondary)",
                                        ...
                                        >
                                            covers {fmt$(t.estimated_coverage_added_dollars)}
                                        </span>
                                    )}
                                </div>
                            </div>
            ...
            <Card
                title="broker_submission_store — today's orders"
                layer="L6"
                status={filledOrders.length > 0 ? "ok" : openOrders.length > 0 ? "warn" : "idle"}
            >
                <Grid cols={4}>
                            ...
                            >
                                ...
                                <div style={{ textAlign: "right", flexShrink: 0 }}>
                                    {o.actual_debit_dollars != null ? (
                                        ...
                                        >
                                            {fmt$(o.estimated_debit_dollars)} est.
                                        </span>
                                    ) : null}
                                    <span
                                        style={{
            ...
            <Card
                title="eod_hedge_engine — today's submission attempts"
                layer="L7"
                status={alerts.length > 0 ? "warn" : "ok"}
            >
                {alerts.length === 0 ? (
                    <div style={{ fontSize: 12, color: "var(--color-text-success)" }}>
                        No EOD alerts today — all spreads within bid-ask thresholds.
                    </div>
                ) : (
                    alerts.map((a, i) => (
                        <div
                        ...
                        >
                            <div style={{ display: "flex", gap: 8, marginBottom: 2 }}>
                                <Badge
                                    type="warn"
                                    label={a.alert_type === "wide_spread" ? "Wide spread" : "No fill"}
                                />
                                <span
                                    style={{
                                        fontSize: 11,
                                ...
                                {a.width_pct != null && (
                                    <span style={{ fontSize: 11, color: "var(--color-text-secondary)" }}>
                                        {(a.width_pct * 100).toFixed(1)}% wide
                                    </span>
                                )}
                            </div>
                            <div style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>
                ...
                >
                    <div style={{ fontSize: 11, color: "var(--color-text-secondary)" }}>
                        Scheduled today: 3:00 monitor → 3:15/25/35 mid price → 3:45 ask+slippage → 4:30 snapshot
                    </div>
                </div>
            </Card>
            ...
            <Card
                title="hedge_snapshot_writer — DB history"
                layer="L8"
                status={histRows.length > 0 ? "ok" : "warn"}
            >
                <Grid cols={3}>
                    ...
                    <Metric
                        label="Latest hedge %"
                        value={histRows.length > 0 ? fmtPct(histRows.at(-1)?.current_hedge_pct) : "—"}
                    />
                </Grid>
                            ...
                            .map((r, i) => (
                                ...
                                >
                                    ...
                                    <span style={{ fontSize: 12, fontFamily: "var(--font-mono)" }}>
                                        {fmtPct(r.current_hedge_pct)} hedge
                                    </span>
                        ...
                        {crashScenarios.map((s, i) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        loss: {fmt$(s.portfolio_loss_dollars)}
                                    </span>
                                    <div
                                        style={{
                                            flex: 1,
            ...
            <Card
                title="hedge_roll_engine — hold / roll / replace recommendations"
                layer="L4"
                status={roll.primary_decision ? "ok" : "warn"}
            >
                <Grid cols={2}>
                    <div>
                        <div
                ...
                {[
                    {
                        ...
                        msg:
                            intel._error ||
                            `QQQ spot: ${fmtN(context.qqq_spot, 2)} · regime: ${regime} · beta: ${fmtN(intel.portfolio_beta, 3)}`,
                    },
                    {
                        layer: "L2",
                        label: "Intelligence",
                        ...
                        msg:
                            intel._error ||
                            `Gap: ${fmtPct(intel.additional_hedge_pct)} (${fmt$(intel.additional_hedge_exposure_dollars)}) · budget: ${fmt$(intel.remaining_hedge_budget_dollars)}`,
                    },
                    {
                        layer: "L3",
                        label: "Option selector",
                        check: !!sel.primary_spread,
                        msg: sel.primary_spread
                            ? `Primary: ${sel.primary_spread.long_leg?.symbol || "—"} · Tail: ${sel.tail_spread?.long_leg?.symbol || "—"}`
                            : "No spread selected",
                    },
                    {
                        layer: "L4",
                        label: "Execution plan",
                        check: !!plan.primary_spread,
                        msg: plan.primary_spread
                            ? `${plan.primary_spread.contracts || 0} primary + ${plan.tail_spread?.contracts || 0} tail contracts · ${fmt$(plan.total_estimated_cost_dollars)} total debit`
                            : "No plan",
                    },
                    {
                        layer: "L4",
                        ...
                        check: true,
                        msg: `Primary: ${primaryAction} · Tail: ${tailAction} · ${(recon.immediate_actions || []).length} immediate actions`,
                    },
                    {
                        layer: "L5",
                        label: "Tickets",
                        check: true,
                        msg: `${ticketList.length} tickets: ${executableTickets.length} buy_spread, ${holdTickets.length} hold_existing`,
                    },
                    {
                        layer: "L6",
                        label: "Orders",
                        check: true,
                        msg: `${allOrders.length} orders total · ${filledOrders.length} filled · ${openOrders.length} open · ${fmt$(orders.total_actual_debit_dollars || 0)} spent`,
                    },
                    {
                        layer: "L7",
                        label: "EOD engine",
                        ...
                        msg:
                            alerts.length === 0
                                ? "All attempts OK, no alerts"
                                : `${alerts.length} alert(s): ${alerts.map((a) => a.alert_type).join(", ")}`,
                    },
                    {
                        layer: "L8",
                        label: "History",
                        check: histRows.length > 0,
                        msg:
                            histRows.length > 0
                                ? `${histRows.length} snapshots · latest: ${histRows.at(-1)?.date || "—"} at ${fmtPct(histRows.at(-1)?.current_hedge_pct)}`
                                : "No snapshots in DB",
                    },
                ].map((r, i) => (
                    <div
                    ...
                    >
                        ...
                        <span
                            style={{
                                ...
                                height: 6,
                                borderRadius: "50%",
                                background: r.check ? "#16A34A" : "#DC2626",
                                flexShrink: 0,
                                marginBottom: 1,
                            }}
                        />
